...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// Declaring a float total to store sum of unnormalized probabilities

	float total = 0.0;

	//Traversing through the grid vector to calculate the sum of the probability
    ...
    //Running another loop to populate the newGrid vector with normalized probabilities
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	//Three floating point variables to calculate blurring factor for the cells

	float center_prob = 1.0 - blurring;
	float corner_prob = blurring/12.0;
	float adjacent_prob = blurring/6.0;

	//Populating the Unnormalized Matrix

	newGrid[0][0] = corner_prob;
	newGrid[0][2] = corner_prob;
	newGrid[2][0] = corner_prob;
	newGrid[2][2] = corner_prob;

	newGrid[0][1] = adjacent_prob;
	newGrid[1][0] = adjacent_prob;
	newGrid[2][1] = adjacent_prob;
	newGrid[1][2] = adjacent_prob;

	newGrid[1][1] = center_prob;

	//Implementing the Normalize function on the newGrid Matrix
